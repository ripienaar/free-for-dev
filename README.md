...
## APIs, Data, and ML
  ...
  * [AGPC Domain Check](https://guild.tradeuniquecapital.com/api) - Check a domain's email authentication — SPF (with the 10-lookup limit), DKIM selectors, DMARC enforce-vs-monitor, MX/null-MX — graded A-F with a permanent shareable report page per check. Also an MCP server for AI agents. Free, no key, no signup, 20 checks per caller per hour.
