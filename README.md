...
## Monitoring
  ...
  * [MetricsHub](https://metricshub.com) — MetricsHub is an OpenTelemetry collection agent that extracts metrics from any platform, application, or service using protocols like SSH, WMI, WBEM, WinRM, SNMP, or HTTP. It seamlessly pushes these metrics to any observability platform that supports OpenTelemetry, such as Datadog, Prometheus, Grafana, Splunk, or New Relic. MetricsHub is available in two editions: Community, for basic infrastructure monitoring, and Enterprise, offering full coverage for advanced monitoring needs.
