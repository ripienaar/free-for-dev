...
## APIs, Data, and ML
  ...
  * [fastCRW](https://fastcrw.com/) - Scrape, crawl, map, search, and extract endpoints that return LLM-ready markdown or structured JSON, with JavaScript rendering, anti-bot handling, and proxy rotation done server side. Accepts the Firecrawl API shape, so an existing client works by pointing at a different base URL. The free plan includes 1000 credits, one per page, as a one-time allocation rather than a monthly reset, with no credit card required. The engine is AGPL and can be self-hosted at no cost.
