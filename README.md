...
## APIs, Data, and ML
  ...
  * [BMObot](https://bmobot.ai) — 31 free developer tool APIs covering hashing, encoding, JSON/CSV/YAML formatting, regex testing, cron parsing, JWT decoding, QR code generation, UUID/ULID, SQL formatting, and more. Completely free with no signup, no API key, and no rate limits. Every service includes an interactive playground, OpenAPI specs, and Swagger docs. Also available as a CLI (`npx bmobot`) and MCP server for Claude.
