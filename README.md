...
## APIs, Data, and ML
  ...
  * [AI Economics Tools API](https://piszczek.pl/tools/api) - Twelve AI-economics calculators as a free JSON API: token cost across vendors, context-window sizing, agent-hour cost, model-routing savings, LLM energy and CO2, joules per verified task, revocation exposure, Proof-Adjusted Autonomy, verification bottleneck and proof debt. No key, no sign-up, CORS open, stateless; responses include the formula and a plain-language interpretation. An MCP server (`@michalpiszczek/ai-economics-mcp`) exposes the same tools to AI assistants.
