...
## APIs, Data, and ML
  ...
  * [Quant Data](https://quantdata.uk/mcp) - Measured market statistics for AI agents over REST and a remote MCP server: day-type probabilities, volume-wave events with measured win rates, options max pain, and dealer gamma (GEX) for US stocks/ETFs and futures. Every published number traces to a stated measurement. The free tier is 10 calls per day on every endpoint with no key, no signup, and no card; the nightly max pain and GEX dashboards are free without limit.
